...
import sys
import robot.hexapod_sim.sim_api.sim as vrep
import time
import numpy as np
...
class RobotHAL:
    ...
    def stop_simulation(self):
        ...
        if errorCode == vrep.simx_return_ok or errorCode == vrep.simx_return_novalue_flag:
            raise Exception("Simulation could not be stopped")
        return
    ...
    def close_servo_position_streams(self):
        for servoID in range(0, 18):
            if not self.joints_first[servoID]:
                vrep.simxGetJointPosition(self.clientID, self.servos[servoID-1], vrep.simx_opmode_discontinue)
    ...
    def set_joint_torque(self, servoID, torque):
        errorCode = vrep.simxSetJointForce(self.clientID, self.servos[servoID-1], torque, vrep.simx_opmode_streaming)
        assert errorCode == vrep.simx_return_ok or errorCode == vrep.simx_return_novalue_flag, "Failed to set servo torque"
